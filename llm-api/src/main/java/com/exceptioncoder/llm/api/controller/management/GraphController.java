...
public class GraphController {
    ...
    /** 获取 Graph 下关联的 Agent 列表 */
    @GetMapping("/{graphId}/agents")
    public ResponseEntity<List<AgentDefinition>> getGraphAgents(@PathVariable String graphId) {
        return ResponseEntity.ok(graphOrchestrationUseCase.getGraphAgents(graphId));
    }

    /** 获取 Graph 的调用链 */
    @GetMapping("/{graphId}/call-chain")
    public ResponseEntity<List<GraphOrchestrationUseCase.CallChainStep>> getGraphCallChain(
            @PathVariable String graphId) {
        return ResponseEntity.ok(graphOrchestrationUseCase.getGraphCallChain(graphId));
    }
